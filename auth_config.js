...
// -------------------------
function initTheme() {
  const saved = localStorage.getItem('mvk_theme') || (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light');
  applyTheme(saved);
}
...
function applyTheme(theme) {
  ...
  localStorage.setItem('mvk_theme', theme);
...
function updateThemeButton(theme) {
  ...
  if (btn) {
    btn.innerHTML = theme === 'dark' ? '<i class="fas fa-sun" style="color: #fbbf24;"></i>' : '<i class="fas fa-moon"></i>';
...
document.addEventListener('DOMContentLoaded', () => {
  const saved = localStorage.getItem('mvk_theme') || 'light';
